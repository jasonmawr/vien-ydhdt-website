...
// Category mapping: store Vietnamese value, display translated
const CATEGORY_VALUES = {
  all: "Tất cả",
  traditional: "Y học cổ truyền",
  health: "Sức khỏe & Dinh dưỡng",
  institute: "Hoạt động Viện",
  research: "Nghiên cứu khoa học",
  patient: "Hướng dẫn bệnh nhân"
};
...
export default function TinTucPage() {
  ...
  const fetchNews = async () => {
    setIsLoading(true);
    try {
      const categoryValue = activeCategoryKey === 'all' ? "Tất cả" : CATEGORY_VALUES[activeCategoryKey as keyof typeof CATEGORY_VALUES];
      const res = await getPosts(categoryValue, search, 20, 0);
      setPosts(res.data);
    } catch (error) {
      console.error(error);
    } finally {
                    ...
                    <div className="flex flex-col md:flex-row">
                      ...
                      <div className="md:w-1/2 p-10 md:p-14 text-white flex flex-col justify-center">
                        ...
                        <div className="flex items-center gap-4 text-white/70 text-sm mb-8">
                          ...
                          <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {posts[0].view_count} {t('views', { count: posts[0].view_count })}</span>
                        </div>
